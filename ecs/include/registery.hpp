...
private:
    ...
    std::unordered_map<std::type_index, std::function<void(registry&, const entity&)>> _erase_functions;
...
public:
    ...
    const sparse_array<Component>& get_components() const {
        std::type_index type_idx(typeid(Component));
        return std::any_cast<const sparse_array<Component>&>(_components_arrays.at(type_idx));
    }

    template<typename Component>
    typename sparse_array<Component>::reference_type add_component(const entity& to, Component&& c) {
        return get_components<Component>().insert_at(to, std::forward<Component>(c));
    }

    template<typename Component, typename... Params>
    typename sparse_array<Component>::reference_type emplace_component(const entity& to, Params&&... p) {
    ...
    void add_system(Function&& f) {
        _systems.emplace_back([f = std::forward<Function>(f)](registry& reg, float dt) {
