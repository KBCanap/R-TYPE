    ...
    struct drawable {
        ...
        drawable(render::Color color = render::Color::White(), float size = 50.0f)
            : color(color), size(size), texture(nullptr), sprite(nullptr),
              sprite_rect(), use_sprite(false), scale(1.0f), tag(""), texture_path("") {}

        drawable(const std::string& tex_path, render::IntRect rect = render::IntRect(), float scale = 2.0f, const std::string& tag = "")
            : color(render::Color::White()), size(50.0f), texture(nullptr), sprite(nullptr),
              sprite_rect(rect), use_sprite(true), scale(scale), tag(tag), texture_path(tex_path) {}
    };
