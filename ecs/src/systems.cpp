...
void render_system(registry &r, sparse_array<component::position> &positions,
    ...
    for (size_t i = 0; i < backgrounds.size(); ++i) {
        ...
        if (bg && bg->texture) {
            ...
            sprite2->setPosition(
                bg->offset_x + static_cast<float>(window_size.x), 0);
            sprite2->setScale(
                static_cast<float>(window_size.x) / texture_size.x,
                static_cast<float>(window_size.y) / texture_size.y);
        ...
        if (pos && draw) {
            if (draw->use_sprite) {
                if (!draw->texture && !draw->texture_path.empty()) {
                    ...
                    draw->sprite =
                        std::shared_ptr<render::ISprite>(window.createSprite());
                ...
                if (draw->sprite) {
                    ...
                    if (i < animations.size() && animations[i] &&
                        ...
                        draw->sprite->setTextureRect(
                            animations[i]
                                ->frames[animations[i]->current_frame]);
                    } else if (draw->sprite_rect.width > 0 &&
                ...
                auto shape = window.createRectangleShape(
...
void collision_system(registry &r, sparse_array<component::position> &positions,
    ...
    for (size_t proj_idx = 0; proj_idx < projectiles.size(); ++proj_idx) {
        ...
        for (size_t target_idx = 0;
             target_idx < std::min(positions.size(), drawables.size());
             ++target_idx) {
            ...
            if (target_idx < hitboxes.size() && hitboxes[target_idx]) {
                ...
                if (target_drawable->use_sprite &&
                    ...
                    target_width = target_drawable->sprite_rect.width *
                                   target_drawable->scale;
                    target_height = target_drawable->sprite_rect.height *
                                    target_drawable->scale;
                } else {
            ...
            if (collision) {
                ...
                bool is_enemy = target_drawable->tag == "enemy" ||
                                ...
                                target_drawable->tag == "enemy_spread" ||
                    ...
                    if (target_idx < healths.size() && healths[target_idx]) {
                        ...
                        if (projectile->friendly && is_enemy) {
                            for (size_t score_idx = 0;
                                ...
                                if (score && score_idx < drawables.size() &&
                                    drawables[score_idx] &&
    ...
    for (size_t player_idx = 0;
        ...
        if (!player_pos || !player_drawable || !player_hitbox ||
            player_drawable->tag != "player")
            continue;

        for (size_t enemy_idx = 0;
             enemy_idx < std::min(positions.size(), drawables.size());
             ++enemy_idx) {
            ...
            if (!enemy_pos || !enemy_drawable || !enemy_hitbox ||
                (enemy_drawable->tag != "enemy" &&
                 enemy_drawable->tag != "enemy_zigzag" &&
                 enemy_drawable->tag != "boss"))
                continue;
            ...
            bool collision =
                (player_left < enemy_right && player_right > enemy_left &&
    ...
    for (const auto &explosion_pos : explosion_positions) {
        ...
        r.add_component<component::position>(
            explosion_entity,
            component::position(explosion_pos.first, explosion_pos.second));
    ...
    for (size_t i = 0; i < std::min({projectiles.size(), positions.size(),
        ...
        if (projectile && pos && vel) {
            ...
            if (pos->x < -50.0f ||
                ...
                pos->y < -50.0f ||
                pos->y > static_cast<float>(window_size.y) + 50.0f) {
                entities_to_kill.push_back(i);
                continue;
            }
        ...
        if (health) {
            ...
            if (health->current_hp <= 0) {
                ...
                if (i < drawables.size() && drawables[i] &&
                    ...
                    for (size_t score_idx = 0; score_idx < scores.size();
                         ++score_idx) {
                        auto &score = scores[score_idx];
                        if (score && score_idx < drawables.size() &&
                            drawables[score_idx] &&
