...
void weapon_system(registry &r, sparse_array<component::weapon> &weapons,
    ...
    for (size_t i = 0; i < std::min({weapons.size(), positions.size()}); ++i) {
        ...
        bool has_ai_input =
            (i < ai_inputs.size()) && ai_inputs[i] && ai_inputs[i]->fire;
        ...
        if (weapon && pos && can_fire_input) {
            ...
            bool is_burst_start = (weapon->current_burst == 0) &&
                                  (time_since_last_shot >= min_interval);
            bool is_burst_continue =
            ...
            if (weapon->is_burst) {
                weapon->current_burst *=
                    !should_reset_burst; // Reset to 0 if should_reset

                can_fire = is_burst_start | is_burst_continue;

                weapon->last_shot_time =
                    is_burst_start ? current_time : weapon->last_shot_time;
                weapon->last_burst_time = (is_burst_start | is_burst_continue)
                                              ? current_time
                                              : weapon->last_burst_time;
                weapon->current_burst += (is_burst_start | is_burst_continue);
            } else {
                can_fire = time_since_last_shot >= min_interval;
                weapon->last_shot_time =
                    can_fire ? current_time : weapon->last_shot_time;
            }
