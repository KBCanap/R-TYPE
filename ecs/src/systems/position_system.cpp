...
void position_system(registry &r, sparse_array<component::position> &positions,
    ...
    sparse_array<component::ai_input> &ai_inputs =
        r.get_components<component::ai_input>();
    ...
    for (size_t i = 0;
        ...
        // Apply bouncing logic if boss has AI with non-zero base_speed OR has
        // vertical velocity
        bool should_bounce = false;
        if (i < ai_inputs.size() && ai_inputs[i]) {
            should_bounce = (ai_inputs[i]->movement_pattern.base_speed > 0.1f);
        }
        // Also enable bounce if boss has vertical velocity (for simple up-down
        // movement)
        ...
        if (is_boss & has_components & should_bounce) {
            ...
            if (pos->y <= min_y && vel->vy < 0) {
                std::cout << "[PositionSystem] Boss hit top, bouncing down. y="
                          << pos->y << " vy=" << vel->vy << std::endl;
                vel->vy = -vel->vy; // Bounce down
            } else if (pos->y >= max_y && vel->vy > 0) {
                std::cout << "[PositionSystem] Boss hit bottom, bouncing up. y="
                          << pos->y << " vy=" << vel->vy << " max_y=" << max_y
                          << std::endl;
