    ...
    for (size_t i = 0;
        ...
        if (ctrl && vel && input) {
            ...
            if (i < animations.size() && animations[i] &&
                ...
                static const AnimationState anim_up_transition = {
                    true, 3, false, false, 0.0f};
                static const AnimationState anim_up_hold = {false, 4, false,
                                                            false, 0.0f};
                static const AnimationState anim_down = {false, 3, false, false,
                                                         0.0f};
                static const AnimationState anim_neutral = {false, 2, true,
                                                            false, 0.0f};
                ...
                bool holding_up = is_moving_up && !transitioning_to_up &&
                                  (anim->current_frame >= 4);

                // Index: 0=neutral, 1=down, 2=up_hold, 3=up_transition
                static const AnimationState *state_table[4] = {
                    &anim_neutral, &anim_down, &anim_up_hold,
                    &anim_up_transition};

                int idx = transitioning_to_up * 3 + holding_up * 2 +
                          is_moving_down * 1;
                ...
                anim->current_time =
                    apply_extra ? state->time : anim->current_time;
