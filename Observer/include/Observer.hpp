...
public:
    ...
    void subscribe(const std::string& eventType, std::shared_ptr<Observer> observer);
    void unsubscribe(const std::string& eventType, std::shared_ptr<Observer> observer);
...
private:
    ...
    std::unordered_map<std::string, std::vector<std::weak_ptr<Observer>>> event_subscribers_;
};
...
extern "C" {
    ...
    ObserverLib::Observer* create_function_observer(void (*callback)(const ObserverLib::Event*));
    void destroy_observer(ObserverLib::Observer* observer);
    void subject_add_observer(ObserverLib::Subject* subject, ObserverLib::Observer* observer);
    void subject_remove_observer(ObserverLib::Subject* subject, ObserverLib::Observer* observer);
    void subject_notify(ObserverLib::Subject* subject);
    void subject_notify_with_data(ObserverLib::Subject* subject, const void* data, size_t size);
    ObserverLib::EventDispatcher* get_event_dispatcher();
    void dispatcher_subscribe(ObserverLib::EventDispatcher* dispatcher, const char* eventType, ObserverLib::Observer* observer);
    void dispatcher_dispatch(ObserverLib::EventDispatcher* dispatcher, const char* eventType);
}
