...
PlayerManager::PlayerManager(registry& reg, sf::RenderWindow& win)
    : _registry(reg), _window(win) {}

std::optional<entity> PlayerManager::createPlayer(float relativeX, float relativeY, float speed) {
    auto player = _registry.spawn_entity();

    _registry.add_component<component::position>(player, component::position(getRelativeX(relativeX), getRelativeY(relativeY)));
    _registry.add_component<component::velocity>(player, component::velocity(0.f, 0.f));
    _registry.add_component<component::drawable>(player, component::drawable("assets/sprites/r-typesheet42.gif", sf::IntRect(0, 0, 33, 17), 2.0f, "player"));
    _registry.add_component<component::controllable>(player, component::controllable(speed));

    // Arme RAPID par défaut : cadence élevée, dégâts faibles, projectile unique
    _registry.add_component<component::weapon>(player,
    ...
    _registry.add_component<component::hitbox>(player, component::hitbox(66.0f, 34.0f, 15.0f, 0.0f));
    ...
    auto& player_anim = _registry.add_component<component::animation>(player, component::animation(0.2f, true));
...
void PlayerManager::updatePlayerPosition(std::optional<entity>& player, float relativeX, float relativeY) {
    if (player) {
...
void PlayerManager::changePlayerWeaponToSingle(const std::optional<entity>& player) {
    if (!player) return;
...
void PlayerManager::changePlayerWeaponToRapid(const std::optional<entity>& player) {
    if (!player) return;
    auto& weapons = _registry.get_components<component::weapon>();
    auto& player_weapon = weapons[*player];
    if (!player_weapon) return;
...
void PlayerManager::changePlayerWeaponToBurst(const std::optional<entity>& player) {
    if (!player) return;
    auto& weapons = _registry.get_components<component::weapon>();
    auto& player_weapon = weapons[*player];
    if (!player_weapon) return;
...
void PlayerManager::changePlayerWeaponToSpread(const std::optional<entity>& player) {
    if (!player) return;
    auto& weapons = _registry.get_components<component::weapon>();
    auto& player_weapon = weapons[*player];
    if (!player_weapon) return;
