...
void PauseMenu::createButtons() {
    ...
    float titleY =
        static_cast<float>(_windowSize.y) * 0.25f - titleBounds.height / 2.f;
    ...
    _continueButtonText->setCharacterSize(
    ...
    _accessibilityButton->setPosition((_windowSize.x - buttonWidth) / 2.f,
                                      startY + spacing * 2);
    ...
    float accessibilityTextY = startY + spacing * 2 + buttonHeight / 2.f -
    ...
    float quitTextY =
        startY + spacing * 3 + buttonHeight / 2.f - quitBounds.height / 2.f;
...
void PauseMenu::render() {
    if (!_visible) {
        std::cout << "PauseMenu::render() - not visible, returning"
                  << std::endl;
        return;
    }
