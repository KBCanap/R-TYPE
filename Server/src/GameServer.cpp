...
GameServer::GameServer(uint16_t udp_port)
    : _udp_port(udp_port), _next_net_id(1), _game_score(0),
      _enemy_spawn_timer(0.0f), _enemy_spawn_interval(2.0f), _game_time(0.0f),
      _boss_spawned(false), _boss_net_id(0), _game_started(false) {
    try {
        ...
        std::cout << "UDP Game Server started on port " << udp_port
                  << std::endl;
...
void GameServer::startGame(const GameSession &session) {
    ...
    std::cout << "Starting game with " << connected_clients.size() << " players"
              << std::endl;
...
void GameServer::handleUDPPackets() {
    ...
    for (const auto &packet : packets) {
        ...
        if (msg_type == static_cast<uint8_t>(UDPMessageType::CLIENT_PING)) {
            ...
            if (packet.data.size() >=
                8 + 5) { // header + 5 bytes payload (4 timestamp + 1 player_id)
                std::lock_guard<std::mutex> lock(_mutex);

                // Extract player_id from payload (last byte)
                uint8_t player_id =
                    packet.data[8 + 4]; // offset 8 (header) + 4 (timestamp)
                ...
                if (it != _player_id_to_net_id.end()) {
                    ...
                    if (is_new_endpoint) {
                        ...
                        ClientId temp_client_id =
                            static_cast<ClientId>(player_id);
                        _client_endpoints[temp_client_id] =
                            packet.client_endpoint;
                    ...
                    std::cerr << "[GameServer] Unknown player_id: "
                              << static_cast<int>(player_id) << std::endl;
                }
            }
        } else if (msg_type ==
                   static_cast<uint8_t>(UDPMessageType::PLAYER_INPUT)) {
            ...
            if (packet.data.size() >= 8 + 2) { // header + 2 bytes payload
                ...
                std::lock_guard<std::mutex> lock(_mutex);
                for (const auto &[temp_client_id, endpoint] :
                     _client_endpoints) {
                    if (endpoint == packet.client_endpoint) {
                        // temp_client_id is actually player_id (1-4)
                        uint8_t player_id =
                            static_cast<uint8_t>(temp_client_id);
                        ...
                        if (it != _player_id_to_net_id.end()) {
                            ...
                            if (entity_it != _entities.end()) {
                                ...
                                float move_speed =
                                    0.005f; // Increased movement speed
                                if (direction & 0x01)
                                    entity_it->second.pos_y -= move_speed; // UP
                                if (direction & 0x02)
                                    entity_it->second.pos_y +=
                                        move_speed; // DOWN
                                if (direction & 0x04)
                                    entity_it->second.pos_x -=
                                        move_speed; // LEFT
                                if (direction & 0x08)
                                    entity_it->second.pos_x +=
                                        move_speed; // RIGHT
                                ...
                                if (direction & 0x10) {
                                    float cooldown =
                                        0.3f; // 300ms between shots
                                    if (_game_time -
                                            entity_it->second.last_fire_time >=
                                        cooldown) {
                                ...
                                // Clamp positions to 0.0 - 1.0 (relative
                                // coordinates)
                                entity_it->second.pos_x = std::max(
                                    0.0f,
                                    std::min(1.0f, entity_it->second.pos_x));
                                entity_it->second.pos_y = std::max(
                                    0.0f,
                                    std::min(1.0f, entity_it->second.pos_y));
                            }
                        }
                        break;
                    }
...
void GameServer::sendEntityCreate(const ServerEntity &entity,
                                  const std::string &endpoint) {
    auto packet = serializeEntityCreate(entity);
    sendUDPMessage(packet, endpoint);
}

void GameServer::sendPlayerAssignment(uint32_t net_id,
                                      const std::string &endpoint) {
    auto packet = serializePlayerAssignment(net_id);
    sendUDPMessage(packet, endpoint);
}

uint32_t GameServer::generateNetId() { return _next_net_id++; }
    ...
    _socket->async_receive_from(
        ...
        [this](std::error_code ec, std::size_t bytes_received) {
            if (!ec && bytes_received > 0) {
                ...
                packet.data.assign(_recv_buffer.begin(),
                                   _recv_buffer.begin() + bytes_received);
...
void GameServer::sendUDPMessage(const std::vector<uint8_t> &data,
                                const std::string &endpoint) {
    try {
        ...
        uint16_t port =
            static_cast<uint16_t>(std::stoi(endpoint.substr(colon_pos + 1)));
...
// Serialization
std::vector<uint8_t>
GameServer::serializeEntityCreate(const ServerEntity &entity) {
...
std::vector<uint8_t>
GameServer::serializeEntityUpdate(const std::vector<ServerEntity> &entities) {
    std::vector<uint8_t> packet;
...
void GameServer::updateEnemySpawning(float dt) {
    ...
    if (!_boss_spawned && _game_score >= 100) {
        ...
        spawnBoss();
        _boss_spawned = true;
        std::cout << "[GAME] Boss spawned at score " << _game_score
                  << std::endl;
    ...
    if (_enemy_spawn_timer >= _enemy_spawn_interval) {
        ...
        float spawn_y = 0.2f + (static_cast<float>(rand()) / RAND_MAX) *
                                   0.6f; // Random between 0.2 and 0.8

        std::lock_guard<std::mutex> lock(_mutex);

        // Randomly choose enemy type (50% ENEMY, 50% ENEMY_SPREAD) - exactly
        // like solo
        int enemy_type = rand() % 2; // 0 or 1
        if (enemy_type == 0) {
            spawnEnemy(
                0.95f,
                spawn_y); // Enemy with wave movement (weapon_type 0 or 1)
        } else {
            spawnEnemySpread(
                0.95f,
                spawn_y); // Spread enemy with zigzag movement (weapon_type 2)
        }
...
void GameServer::spawnEnemy(float pos_x, float pos_y) {
    ...
    // Randomly choose weapon: 50% single shot (weapon_type 0), 50% burst
    // (weapon_type 1)
    ...
    if (weapon_variant == 0) {
        ...
        enemy.fire_cooldown =
            1.0f + (static_cast<float>(rand()) / RAND_MAX); // 1-2 seconds
    } else {
        // Burst weapon (weapon_type 1 in solo) - Note: burst not implemented
        // yet, use single for now
        enemy.projectile_count = 1;
        enemy.projectile_angle_spread = 0.0f;
        enemy.fire_cooldown =
            1.0f + (static_cast<float>(rand()) / RAND_MAX); // 1-2 seconds
...
void GameServer::spawnEnemySpread(float pos_x, float pos_y) {
    ...
    enemy.fire_cooldown =
        1.0f / 0.8f; // Weapon fire_rate = 0.8, so interval = 1/0.8 = 1.25s
...
void GameServer::spawnBoss() {
    ...
    boss.vel_x = 0.0f;
    boss.vel_y =
        100.0f; // Vertical movement speed 100 pixels/sec (exactly like solo)
    ...
    // Boss movement - straight up/down (bounce handled in AI, exactly like
    // solo)
    ...
    boss.fire_cooldown =
        1.0f / 2.0f; // fire_rate = 2.0, so interval = 0.5 seconds
...
void GameServer::updateEnemyAI(float dt) {
    ...
    // We assume a reference screen size of 1920x1080 for consistent velocity
    // calculations
    ...
    for (auto &[net_id, entity] : _entities) {
        if (entity.type == EntityType::ENEMY ||
            entity.type == EntityType::ENEMY_SPREAD) {
            ...
            if (entity.movement_pattern == MovementPattern::WAVE) {
                // Wave pattern: sinusoidal vertical movement based on X
                // position Exactly like solo: vy = amplitude * sin(frequency *
                // pos_x + phase_offset)
                vel_x_pixels = -entity.pattern_base_speed;
                float pos_x_pixels = entity.pos_x * REFERENCE_WIDTH;
                vel_y_pixels =
                    entity.pattern_amplitude *
                    std::sin(entity.pattern_frequency * pos_x_pixels);
                ...
                vel_x_pixels = -entity.pattern_base_speed;
                float triangle_wave =
                    std::abs(std::fmod(entity.pattern_frequency *
                                           entity.pattern_time,
                                       2.0f) -
                             1.0f) *
                        2.0f -
                ...
                if (entity.projectile_count > 1) {
                    std::cout << "[SHOOT] "
                              << (entity.type == EntityType::ENEMY_SPREAD
                                      ? "ENEMY_SPREAD"
                                      : "ENEMY")
                              << " NET_ID=" << net_id << " firing "
                              << entity.projectile_count << " projectiles with "
                              << entity.projectile_angle_spread << "° spread"
                              << std::endl;
                    spawnMultipleProjectiles(net_id, true,
            ...
            // Bounce at screen edges (exactly like solo: 50px top, 100px bottom
            // margin)
            float top_margin = 50.0f / REFERENCE_HEIGHT;
            float bottom_margin =
                (REFERENCE_HEIGHT - 100.0f) / REFERENCE_HEIGHT;
            ...
            // Boss shooting logic
            if (_game_time - entity.last_fire_time >= entity.fire_cooldown) {
                spawnMultipleProjectiles(net_id, true, entity.projectile_count,
                                         entity.projectile_angle_spread);
                entity.last_fire_time = _game_time;
            }
        }
...
void GameServer::spawnProjectile(uint32_t owner_net_id, bool is_enemy) {
    ...
    projectile.type =
        is_enemy ? EntityType::PROJECTILE : EntityType::ALLIED_PROJECTILE;
    projectile.pos_x =
        owner_it->second.pos_x + (is_enemy ? -0.05f : 0.05f); // Spawn in front
...
void GameServer::spawnMultipleProjectiles(uint32_t owner_net_id, bool is_enemy,
...
void GameServer::updateProjectiles(float /*dt*/) {
    ...
    for (auto &[net_id, entity] : _entities) {
        if (entity.type == EntityType::PROJECTILE ||
            entity.type == EntityType::ALLIED_PROJECTILE) {
            ...
            if (entity.pos_x < -0.1f || entity.pos_x > 1.1f ||
                entity.pos_y < -0.1f || entity.pos_y > 1.1f) {
                _entities_to_destroy.push_back(net_id);
            }
        }
    }
...
bool GameServer::checkCollision(const ServerEntity &a, const ServerEntity &b) {
    return (a.pos_x < b.pos_x + b.width && a.pos_x + a.width > b.pos_x &&
        ...
        for (size_t j = i + 1; j < entities_to_check.size(); ++j) {
            ...
            if (a.type == EntityType::ALLIED_PROJECTILE &&
                (b.type == EntityType::ENEMY ||
                 b.type == EntityType::ENEMY_SPREAD ||
                ...
                _entities_to_destroy.push_back(id_a); // Destroy projectile
                       ...
                       (a.type == EntityType::ENEMY ||
                        a.type == EntityType::ENEMY_SPREAD ||
...
void GameServer::damageEntity(uint32_t net_id, uint32_t damage) {
    ...
    case EntityType::PLAYER:
        entity_type_name = "PLAYER";
        break;
    case EntityType::ENEMY:
        entity_type_name = "ENEMY";
        break;
    case EntityType::ENEMY_SPREAD:
        entity_type_name = "ENEMY_SPREAD";
        break;
    case EntityType::BOSS:
        entity_type_name = "BOSS";
        break;
    case EntityType::PROJECTILE:
        entity_type_name = "ENEMY_PROJECTILE";
        break;
    case EntityType::ALLIED_PROJECTILE:
        entity_type_name = "ALLIED_PROJECTILE";
        break;
    default:
    ...
    if (it->second.health <= damage) {
        ...
        std::cout << "[DAMAGE] " << entity_type_name << " NET_ID=" << net_id
                  << " took " << damage << " damage (" << old_health
                  << " -> 0) - DESTROYED!" << std::endl;
