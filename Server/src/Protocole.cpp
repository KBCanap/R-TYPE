...
ParsedMessage Protocol::parseMessage(const std::string& raw_message)
{
    ...
    if (raw_message.size() < HEADER_SIZE) {
        std::cerr << "Message too short: " << raw_message.size() << " bytes" << std::endl;
        return result;
    }
    ...
    if (!isValidMessageType(msg_type)) {
        std::cerr << "Invalid message type: " << static_cast<int>(msg_type) << std::endl;
        return result;
    }
    ...
    if (raw_message.size() != HEADER_SIZE + data_length) {
        std::cerr << "Invalid message size. Expected: " << HEADER_SIZE + data_length 
...
std::string Protocol::createMessage(MessageType type, const std::vector<uint8_t>& data)
{
    std::string message;
