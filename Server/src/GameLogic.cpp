...
GameLogic::GameLogic(std::shared_ptr<registry> reg)
    : _registry(reg), _running(false), _current_tick(0),
      _game_time(0.0f), _enemy_spawn_timer(0.0f), _enemy_spawn_interval(2.0f),
      _total_score(0), _boss_spawned(false), _boss(0), _next_net_id(1000) {
...
void GameLogic::registerSystems() {
    ...
    _registry->add_system<Position, Hitbox, Projectile, PlayerComponent, Enemy, Health, Score, NetworkComponent>(collisionSystem);
...
void GameLogic::removePlayer(uint client_id) {
    ...
    if (it != _client_to_entity.end()) {
        ...
        std::cout << "[GameLogic] Removed player for client " << client_id << std::endl;
    }
}
...
void GameLogic::spawnEnemy() {
    ...
    _registry->add_component(enemy, NetworkComponent{generateNetId(), true, "enemy"});
...
void GameLogic::spawnBoss() {
    ...
    _registry->add_component(_boss, NetworkComponent{generateNetId(), true, "boss"});
    ...
    _enemies.erase(
        std::remove_if(_enemies.begin(), _enemies.end(),
            [&](entity ent) {
                ...
                return !health_opt || health_opt.value().current_hp <= 0;
            }),
        _enemies.end()
    );
    ...
    _projectiles.erase(
        std::remove_if(_projectiles.begin(), _projectiles.end(),
            [&](entity ent) {
                ...
                if (!pos_opt || !proj_opt)
                    return true;
                return pos_opt.value().x < -50.0f || pos_opt.value().x > 850.0f ||
                       pos_opt.value().y < -50.0f || pos_opt.value().y > 650.0f ||
...
std::vector<EntitySnapshot> GameLogic::getDeltaSnapshot(uint last_acked_tick) {
    auto acked_snapshot = std::find_if(
        _snapshot_history.begin(),
        _snapshot_history.end(),
    ...
    for (const auto& entity : current_snapshot.entities) {
        ...
        if (old_it == old_entities.end()) {
            ...
            float pos_delta = std::sqrt(
                std::pow(entity.pos.x - old.pos.x, 2) +
                std::pow(entity.pos.y - old.pos.y, 2)
            );

            should_send = (pos_delta > POS_THRESHOLD ||
...
void GameLogic::inputSystem(registry &reg,
...
void GameLogic::movementSystem(registry &reg,
...
void GameLogic::weaponSystem(registry &reg,
...
void GameLogic::collisionSystem(registry &reg,
                                sparse_array<Position> &positions,
...
void GameLogic::healthSystem(registry &reg,
                             sparse_array<Health> &healths,
                             sparse_array<NetworkComponent> &network_comps,
                             float dt) {
...
void GameLogic::enemyAISystem(registry &reg,
                              sparse_array<Enemy> &enemies,
                              sparse_array<Position> &positions,
                              sparse_array<Velocity> &velocities,
                              float dt) {
...
void GameLogic::bossAISystem(registry &reg,
