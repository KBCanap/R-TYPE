...
static void show_helper() {
    ...
    std::cout << "./r-type_server [-p <tcp_port>] [-u <base_udp_port>]"
              << std::endl;
    std::cout << "\nDESCRIPTION:" << std::endl;
    std::cout << "  Starts a lobby server that manages multiple game instances."
              << std::endl;
    std::cout << "  Each game instance will use a unique UDP port starting "
                 "from base_udp_port."
              << std::endl;
    std::cout << "\nOPTIONS:" << std::endl;
    std::cout << "  -p <port>    TCP port number for lobby server (1024-65535)"
              << std::endl;
    std::cout
        << "  -u <port>    Base UDP port number for game instances (1024-65535)"
        << std::endl;
    ...
    std::cout << "    - Allocate game instances starting at UDP port 8080, "
                 "8081, 8082..."
              << std::endl;
}
...
static ServerConfig parse_arguments(int ac, char **av) {
    ...
    for (int i = 1; i < ac; i++) {
        if (std::strcmp(av[i], "-p") == 0) {
            ...
            if (i + 1 >= ac || !is_a_valid_port(av[i + 1])) {
                std::cerr
                    << "Error: Invalid or missing base UDP port after -u flag"
                    << std::endl;
                show_helper();
                return config;
            }
