...
StartServer::StartServer(int port, int base_udp_port)
    : _tcp_port(port), _base_udp_port(base_udp_port), _next_server_id(1),
      _protocol(), _game_session() {
...
StartServer::~StartServer() {
    ...
    for (const auto &[lobby_id, instance] : _game_instances) {
        if (instance.process_id != INVALID_PROCESS_ID) {
            std::cout << "Terminating game instance " << instance.server_id
                      << std::endl;
#ifdef _WIN32
            terminateProcess(instance.process_id);
#else
...
#ifdef _WIN32
BOOL WINAPI StartServer::consoleCtrlHandler(DWORD signal) {
...
void StartServer::childSignalHandler(int signal) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        std::cout << "Game instance (PID: " << pid << ") terminated"
                  << std::endl;
    }
}
#endif
...
void StartServer::run() { lobbyLoop(); }
...
void StartServer::processMessage(const ClientMessage &message) {
    ...
    if (!parsed.valid) {
        std::cerr << "[Client " << message.client_id
                  << "] Invalid message format" << std::endl;
    ...
    default:
        std::cerr << "[Client " << message.client_id
                  << "] Unexpected message type" << std::endl;
...
void StartServer::handleConnect(uint32_t client_id,
    ...
    if (username.empty()) {
        std::cerr << "[Client " << client_id << "] Invalid or empty username"
                  << std::endl;
    ...
    if (!_game_session.addClient(client_id, username)) {
        std::cerr << "[Client " << client_id << "] Failed to add client"
                  << std::endl;
        sendError(client_id, ProtocolError::INTERNAL_SERVER_ERROR);
        return;
    }

    std::cout << "[Client " << client_id << "] Connected as '" << username
              << "'" << std::endl;
...
void StartServer::handleLobbyListRequest(uint32_t client_id) {
    std::cout << "[Client " << client_id << "] Requesting lobby list"
              << std::endl;
    ...
    std::cout << "[Client " << client_id << "] Sent " << lobbies.size()
              << " lobbies" << std::endl;
}

void StartServer::handleLobbyInfoRequest(uint32_t client_id,
                                         const std::vector<uint8_t> &data) {
    if (data.size() < 2) {
    ...
    std::cout << "[Client " << client_id << "] Requesting info for lobby "
              << lobby_id << std::endl;
    ...
    std::string response =
        _protocol.createLobbyInfoResponse(lobby_info.value());
    sendToClient(client_id, response);
}

void StartServer::handleCreateLobby(uint32_t client_id,
                                    const std::vector<uint8_t> &data) {
...
void StartServer::handleJoinLobby(uint32_t client_id,
                                  const std::vector<uint8_t> &data) {
    if (data.size() < 2) {
        sendError(client_id, ProtocolError::PROTOCOL_VIOLATION);
        return;
    ...
    std::cout << "[Client " << client_id << "] Joining lobby " << lobby_id
              << std::endl;
    ...
    std::string ack_response =
        _protocol.createJoinLobbyAck(lobby_id, player_id.value(), players);
        ...
        for (const auto &player : lobby->players) {
            if (player.client_id == client_id) {
                ...
                std::string broadcast =
                    _protocol.createPlayerJoined(new_player_info);
...
void StartServer::handleLeaveLobby(uint32_t client_id) {
    ...
    std::cout << "[Client " << client_id << "] Left lobby " << lobby_id
              << std::endl;
...
void StartServer::handleReady(uint32_t client_id) {
    ...
    std::cout << "[Client " << client_id << "] Ready in lobby " << lobby_id
              << std::endl;

    // Check if game can start
    if (_game_session.canStartGame(lobby_id)) {
        std::cout << "[Lobby " << lobby_id
...
#ifdef _WIN32
process_id_t StartServer::createGameProcess(uint16_t server_id,
                                            uint16_t udp_port,
                                            size_t player_count) {
    ...
    // Build command line: game_server.exe <udp_port> <player_count>
    std::string cmd_line = std::string(exe_path) + " " +
...
void StartServer::terminateProcess(process_id_t process_id) {
...
bool StartServer::isProcessRunning(process_id_t process_id) {
    if (process_id == INVALID_PROCESS_ID) {
        return false;
    }

    DWORD exitCode;
    if (GetExitCodeProcess(process_id, &exitCode)) {
...
bool StartServer::startGameInstance(uint16_t lobby_id) {
    auto lobby = _game_session.getLobby(lobby_id);
    if (!lobby.has_value()) {
        std::cerr << "Cannot start game: lobby " << lobby_id << " not found"
                  << std::endl;
        return false;
    }
    ...
    if (!_game_session.startGame(lobby_id)) {
        ...
        std::cerr << "Cannot start game: failed to mark as started"
                  << std::endl;
        return false;
    }
    ...
    process_id_t process_handle =
        createGameProcess(server_id, udp_port, lobby->players.size());

    if (process_handle == INVALID_PROCESS_ID) {
        std::cerr << "Failed to create game process!" << std::endl;
        freeUdpPort(udp_port);
        return false;
    }

    GameInstance instance(lobby_id, server_id, process_handle, udp_port,
                          server_ip);
    ...
    std::cout << "[Lobby Server] Game instance " << server_id
              << " created with process handle" << std::endl;
    ...
    if (pid == 0) {
        ...
        try {
            ...
            game_loop.stop();
            std::cout << "[Game Instance " << server_id << "] Finished"
                      << std::endl;
        } catch (const std::exception &e) {
            std::cerr << "[Game Instance " << server_id
...
void StartServer::cleanupFinishedGames() {
    ...
    for (const auto &[lobby_id, instance] : _game_instances) {
#ifdef _WIN32
        // Windows: Check if process is still running
        if (!isProcessRunning(instance.process_id)) {
            finished_lobbies.push_back(lobby_id);
            CloseHandle(instance.process_id);
        }
#else
        // Unix: Check if process is still running
        int status;
...
void StartServer::broadcastToLobby(uint16_t lobby_id,
...
uint32_t StartServer::getServerIp() const { return htonl(INADDR_LOOPBACK); }